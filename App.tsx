...
const App: React.FC = () => {
  ...
  const [formView, setFormView] = useState<{ type: 'article' | 'course', item?: any } | null>(null);
  const [userState, setUserState] = useState<UserState>(() => 
    storage.get('hk_hub_session', {
      isLoggedIn: false, userPhone: null, userName: null, isAdmin: false, fontSize: 1.2,
    })
  );
  ...
  useEffect(() => {
    ...
    return () => { unsubA(); unsubC(); unsubU(); };
  }, []);

  // 當 usersMap 更新時，同步當前登入用戶的 isAdmin 狀態
  useEffect(() => {
    if (userState.isLoggedIn && userState.userPhone && usersMap[userState.userPhone]) {
  ...
  useEffect(() => { storage.set('hk_hub_session', userState); }, [userState]);
  ...
  const handleLogout = () => {
    setUserState(prev => ({ ...prev, isLoggedIn: false, userPhone: null, isAdmin: false }));
    setActiveTab(0);
  };

  const getUserName = useCallback((phone: string) => {
    return usersMap[phone]?.name || phone;
  }, [usersMap]);

  if (isLoading) return <div className="h-screen flex items-center justify-center bg-blue-50 text-blue-600 italic">載入中...</div>;
  ...
  return (
    <div>
      <Layout activeTab={activeTab} onTabChange={setActiveTab} title={['最新公告', '課程報名', '個人設定'][activeTab]}>
          ...
          {activeTab === 0 && (
            <NewsTab 
              ...
              onToggleLike={(id) => {
                ...
                if (a && userState.userPhone) fb.toggleArticleLike(id, userState.userPhone, a.likedBy.includes(userState.userPhone));
              }}
