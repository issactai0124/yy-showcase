...
const CourseForm: React.FC<CourseFormProps> = ({ item, onClose, onSave }) => {
  ...
  const [date, setDate] = useState(
    item?.date 
      ? new Date(item.date.getTime() - (new Date().getTimezoneOffset() * 60000)).toISOString().slice(0, 16) 
      : getTomorrowAtNine()
  );
    ...
    <div className="fixed inset-0 bg-slate-50 z-50 flex flex-col h-screen overflow-hidden">
      <header className="bg-white border-b border-slate-200 py-4 px-6 flex items-center justify-between sticky top-0 z-10">
        <div className="flex items-center space-x-3">
          ...
          <h1 className="text-xl font-bold text-slate-800">{item ? '修改課程' : '開設新課程'}</h1>
        </div>
        <button 
          onClick={handleSave} 
        ...
        <div className="max-w-4xl mx-auto">
          <div className="bg-white p-6 lg:p-10 rounded-2xl shadow-sm border border-slate-200 space-y-8">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-6">
              <label className="sm:w-28 text-xs font-bold text-slate-400 uppercase tracking-wider shrink-0">課程名稱</label>
              <input 
            ...
            {/* 開課日期與時間 */}
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-6">
              <label className="sm:w-28 text-xs font-bold text-slate-400 uppercase tracking-wider shrink-0">課程日期</label>
              <input 
                type="datetime-local" 
            ...
            <div className="flex flex-col sm:flex-row gap-2 sm:gap-6">
              <label className="sm:w-28 text-xs font-bold text-slate-400 uppercase tracking-wider pt-2 shrink-0">簡介細則</label>
              <textarea 
                rows={12} 
