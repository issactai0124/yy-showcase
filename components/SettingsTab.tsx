...
const SettingsTab: React.FC<SettingsTabProps> = ({ userState, onStateChange, onLogout, onNameChange }) => {
  ...
  return (
    <div className="p-4 pb-20 space-y-8">
      {/* Account Settings */}
      <section>
        <h2 className="text-blue-600 font-bold text-sm uppercase tracking-wider mb-4 px-2">帳戶設定</h2>
          ...
          <div className="p-4 flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-slate-900 font-bold">{userState.userName || '未輸入'}</p>
              </div>
            </div>
            <button 
              onClick={() => {
      ...
      {/* App Settings */}
      <section>
        <h2 className="text-blue-600 font-bold text-sm uppercase tracking-wider mb-4 px-2">應用程式設定</h2>
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 divide-y divide-slate-100 overflow-hidden">
          <div className="p-4 space-y-3">
            ...
            <input 
              ...
              onChange={(e) => onStateChange({ ...userState, fontSize: parseFloat(e.target.value) })}
            />
          </div>
        </div>
      </section>
