...
const NewsTab: React.FC<NewsTabProps> = ({ articles, userState, onOpenForm, onDeleteArticle, onToggleLike, getUserName }) => {
  ...
  return (
    <div className="p-4 pb-20 space-y-8">
      <section>
        <h2 className="text-blue-600 font-bold text-sm uppercase tracking-wider mb-4 px-2">最新發佈</h2>
        ...
        <div className="space-y-4">
          {articles.map(article => (
            <div key={article.id} className="bg-white rounded-2xl shadow-sm border border-slate-200 divide-y divide-slate-100 overflow-hidden">
              <div className="p-4 flex items-center justify-between bg-slate-50/30">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <p className="text-xs font-medium text-slate-500">{formatDate(article.date)}</p>
                    <p className="text-slate-900 font-bold">{article.title}</p>
                  </div>
                </div>
                {userState.isAdmin && (
                  <div className="flex items-center space-x-1">
                    <button onClick={() => onOpenForm(article)} className="text-blue-400 hover:text-blue-600 p-2 transition-colors">
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                    </button>
                    <button onClick={() => setConfirmDeleteId(article.id)} className="text-red-400 hover:text-red-600 p-2 transition-colors">
              ...
              <div className="p-4">
                <p className="text-slate-700 whitespace-pre-wrap text-sm leading-relaxed">{article.content}</p>
                {userState.isAdmin && article.likedBy.length > 0 && (
                  <div className="mt-3 pt-3 border-t border-slate-100">
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter mb-1">已點讚用戶：</p>
                    <p className="text-xs text-slate-500 font-medium italic">
                      {article.likedBy.map(phone => getUserName(phone)).join('、')}
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="px-4 py-3 bg-slate-50/30 flex justify-between items-center">
                ...
                >
                  <svg className={`w-5 h-5 ${article.likedBy.includes(userState.userPhone || '') ? 'fill-current' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" /></svg>
      ...
      {confirmDeleteId && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-2xl">
            <h2 className="text-xl font-bold mb-2 text-slate-900">確認刪除公告？</h2>
            <p className="text-slate-500 text-sm mb-4">此動作無法復原，您確定要刪除這條公告嗎？</p>
